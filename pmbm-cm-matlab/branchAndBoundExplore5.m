function [hyposLocal,hyposCardLocal,probLogLocal,kInvestigate] = branchAndBoundExplore5(hypos,hyposCard,clusters,clustersCard,probLogHypos,iC,assocLocal,gainMatPostC,indicesOfNewbornTracks,nHypoMax,nHypoTotalMax,trackNumberLookup,k)
    ...
    for iH=1:size(parentHyposIndices,2)
        ...
        if(~isempty(tracksH))
            ...
            bestLocal(ii).clustercontrib(counterIH) = struct('tracks',tracksH,'meaIndAll',meaIndH,'meaIndOpt',meaIndH(personToItem),'meaIndOptLocal',personToItem,...
                'parentHypo',parentHyposIndices(iH),'parentPriorScore',probLogHypos(parentHyposIndices(iH)));
...
if(~isempty(bestLocal))
    ...
    if(~isempty(pq))
        ...
        while(uub > lpb && iter <= maxIter) % Should also have a test for whether I actually have more candidates to investigate.
            ...
            % Is UUB holder a solved hypothesis, or is it unsolved?
            
            if(isempty(pq(uubHolder).score))
                
                error('stop here for now, and fix it when I get here');
            end
         
            
            % Do the AS Murty again
...
%                 if(iter > 2)
%                    ixu = find(vertcat(pq.labelHypo) == 8); 
%                     disp(['At iter ',num2str(iter),' the switchability is ',num2str(pq(ixu).clustercontrib(3).switchable)]);
%                 end
...
%                                 test = testPQLookup(sMarked,trackNumberLookup);
%                                 if(any(test))
%                                    error('nan trouble'); 
%                                 end
...
%                             if(sum(losses) >= lpb && sMarked.labelHypo == 40)
%                                 disp('found 20')
%                                 asMurtyOrder
%                                 p.labelHypo
%                                 %error('stop here');
%                             end
%                             if(sum(losses) >= lpb && sMarked.labelHypo == 408)
%                                 disp('found 312')
%                                 error('stop here');
%                             end
...
%                                 ixu = find(vertcat(pq.labelHypo) == 8);
%                                 
%                                 disp(['At iter ',num2str(iter),' and sIter ',num2str(sIter),' the switchability is of HL 19 is ',num2str(pq(ixu).clustercontrib(3).switchable)]);
%                                 %disp(['Label of psEntry = ',num2str(psEntry),'is ',num2str(pq(psEntry).labelHypo)]);
%                             end                        
                    ...
                    %[bounds,uubHolder] = boundRecurse2(topNode,pq);
...
%                 if(iter > 2)
%                    ixu = find(vertcat(pq.labelHypo) == 8); 
%                     disp(['At iter ',num2str(iter),' the switchability is ',num2str(pq(ixu).clustercontrib(3).switchable),' after AS']);
%                 end                
...
%                 if(iter > 2)
%                    ixu = find(vertcat(pq.labelHypo) == 8); 
%                     disp(['At iter ',num2str(iter),' the switchability is ',num2str(pq(ixu).clustercontrib(3).switchable),' before lazy switch']);
%                 end            
%             
...
%                                 end                            
                            ...
                            pq(uubHolder).childrenCount = pq(uubHolder).childrenCount + 1;
                                ...
                                if(pq(end).labelHypo == 30 && k==621 && iC==1)
                                    
                                   % I need a contention check function here
                                   
                                   %[contentionClusters,contentionTracks] = contentionCheck(sMarked,bestLocal)
                                   %error('study HL133'); 
                                end                            
                            if(any(~testBAndCpqI(sMarked)))
                                error('Something weith wrong with B and C level indices');
            ...
            % attempt more abstract expansion loop
            
%                 if(iter > 2)
%                    ixu = find(vertcat(pq.labelHypo) == 8); 
%                     disp(['At iter ',num2str(iter),' the switchability is ',num2str(pq(ixu).clustercontrib(3).switchable),' before expansion']);
%                 end            
            ...
            for ii=1:nCL
                ...
                if(any(pq(uubHolder).clustercontrib(ii).expandable))
                    expandableTrackIndices = find(pq(uubHolder).clustercontrib(ii).expandable);
                    lossbounds = zeros(size(expandableTrackIndices));
                    
                    
                  
                    for tt=1:length(expandableTrackIndices)
                        tInSuper = p.clustercontrib(ii).tracksLocalInSuper(expandableTrackIndices(tt));
                        tRow = p.rewardMatrix(tInSuper,:);
                        bestReward = tRow(p.clustercontrib(ii).meaIndOptLocalInSuper(expandableTrackIndices(tt)));
                        tRow(p.clustercontrib(ii).meaIndOptLocalInSuper(expandableTrackIndices(tt))) = superMinVal;
                        [secondBestReward,maximizer] = max(tRow);
                        lossbounds(tt) = secondBestReward-bestReward;
                        solvable = false;
                        
                        
                  
                                                                     
                        if(lossbounds(tt) + pq(uubHolder).score > lpb)
                            ...
                            if(~isempty(pqO) )
                                ...
                                if(pqO.bound >= lpb)
                                    %                                 test = testPQLookup(pqO,trackNumberLookup);
                                    %
                                    %                                 if(any(test))
                                    %                                    error('nan trouble');
                                    %                                 end
                            ...
                            pq(uubHolder).clustercontrib(ii).expandable(expandableTrackIndices(tt)) = false;
                        end
            ...
            worseThanOldUUB = find(newValueList < uub);
        ...
        if(test1 && test2 && test3 && test4 && test5 && k>=874 && test6 && test7)
    ...
    %     if(checkDuplicates(hyposLocal,hyposCardLocal,1:length(hyposCardLocal),length(hyposCardLocal)))
    %        error('got duplicates');
    %     end
