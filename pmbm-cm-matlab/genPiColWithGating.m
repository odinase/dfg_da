function piCol = genPiColWithGating(h,m,maxM,zBarH,sBarH,measurements,gammaGate)
...
for s=1:nSlots
    ...
    for ii=0:m
        colS = collection;
        if(ii> 0)
            innov = zBarH(:,s) - measurements(:,ii);
            gateStatistic = innov'*(covVec2Mat(sBarH(:,s))\innov);
            gateTest = gateStatistic < gammaGate;
            if(gateTest)
                colS(:,s) = ii;
                iBool = colS == ii;
