function [hyposLocal,hyposCardLocal,probLogLocal,kInvestigate,nCL,pcSize] = doubleMurtyExplore(hypos,hyposCard,clusters,clustersCard,probLogHypos,iC,assocLocal,gainMatPostC,indicesOfNewbornTracks,nHypoMax,nHypoTotalMax,trackNumberLookup,nCap,mCap,k)
...
[assignments,costs]= murty(-rewardFC,nCap);
...
% 
% 
% % Stuff from branchAndBoundExplore6
% 
...
%             newParents = zeros(1,length(pq));
%             for ii=1:length(pq)
%                 newParents(oldParents==ii) = find(ixV==ii); % THIS FIND IS CAUSING A LOT OF EXTRA RUNTIME
%             end
%             np = newParents(ixV);
%             
%             % Store re-ordered tree parents
