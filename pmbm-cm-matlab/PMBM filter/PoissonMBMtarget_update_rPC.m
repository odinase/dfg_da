function [filter_upd,cList]=PoissonMBMtarget_update_rPC(filter_pred,z,p_d,k,gating_threshold,intensity_clutter,Nhyp_max,doMechi,system,params)

%Author: Angel F. Garcia-Fernandez
% rPC (rMat Polar-converted) version revised by Edmund Brekke November 2021
...
for m=1:size(z,2)
    ...
    for i=1:length(filter_pred.weightPois)
        ...
        if(doMechi)
            own2tar = mean_i(1:2) - params.stateFullOwn(1:2,k);
...
%        indices_new_tracks
%        
%        filter_upd.tracks{Nprev_tracks+m}.meanB{1} 
%     end          
...
if(Nprev_tracks==0)
    ...
    for p=1:length(filter_pred.globHypWeight)
        ...
        if(isempty(cost_matrix_log_trimmed))
            ...
            %'bm'
            [opt_indices,nlcost,cListM]=murty(-cost_matrix_log_trimmed',kbest);  
