...
alive_targets_pos_index=[alive_targets_index;alive_targets_index;false(1,length(t_birth));false(1,length(t_birth))];
X_pos_truth_k=reshape(X_truth(alive_targets_pos_index(:),k),2,Nalive_targets);


nXEst = size(X_estimate,1)/4;

X_estimate_pos_reshape_full = reshape(X_estimate,[4,nXEst]);
X_estimate_pos_reshape = X_estimate_pos_reshape_full(1:2,:);
