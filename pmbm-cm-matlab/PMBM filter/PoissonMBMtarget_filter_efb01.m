...
% EFB revision 01: Make ready to include measurements from Devijver example
...
P_ini=diag([(xMax-xMin)^2/12 (yMax-yMin)^2/12 pInitVel(1) pInitVel(1) ]);
...
lambda0=muBirth; % EFB: I believe this is an intensity, and not a rate.
...
for i=1:Nmc
    ...
    for tt=1:numtruth
        kList = truth(i).targetsTrue(tt).k;
        for kI=1:length(kList)
            k = kList(kI);
            X_truth(4*tt-3:4*tt,k)=truth(i).targetsTrue(tt).x(:,kI);
        end
        t_birth(tt) = kList(1);
