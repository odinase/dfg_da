function [hyposLocal,hyposCardLocal,probLogLocal,kInvestigate,pqLen,priorCardAve,pq] = branchAndBoundExplore(hypos,hyposCard,clusters,clustersCard,probLogHypos,iC,assocLocal,gainMatPostC,indicesOfNewbornTracks,nHypoTotalMax,trackNumberLookup,k)
...
if(~isempty(bestLocal))
    ...
    if(~isempty(pq))
        ...
        while(uub > lpb && iter <= maxIter) % Should also have a test for whether I actually have more candidates to investigate.
            ...
            %bestLocalUUBHolder = horzcat(pq(uubHolder).clustercontrib.parentInBestList)
            %measPU = pq(uubHolder).meaEntireOpt
            
            %labelUUBHolder = pq(uubHolder).labelHypo
            
            %disp(['LPB is ',num2str(lpb),' and UUB is ',num2str(uub)]);
            %horzcat(pq.labelHypo)
...
%                     pBound = p.bound
%                     pScore = p.score
%                     uubHolderBound = pq(uubHolder).bound
%                     uubHolderScore = pq(uubHolder).score
...
%                         if(iter == 3)
%                               %tt
%                                 pq(uubHolder).tracksEntire(trackOrder(tt))
%                                 
%                                 if(~isempty(pqO))
%                                     pqO.labelHypo    
%                                 else
...
%             if(iter==3)
%                error('stop in iteration 1'); 
%             end
            
            
            %error('stop after itreation 1');
            ...
            uubHistPure = [uubHistPure,uub];
            lpbHistPure = [lpbHistPure,lpb];
        ...
        priorCardList = zeros(1,length(pq));
        for ii=1:length(priorCardList)
            priorCardList(ii) = length(pq(ii).tracksEntire);
        end
        priorCardAve = mean(priorCardList);
        
        
%         if(k==610 && nCL > 1)
%             figure;
%             plot(uubHistPure,'r','linewidth',1);
%             hold on;
%             plot(lpbHistPure,'k');
%             hold off;
%             legend('UUB','LSB');
%             axis([0,length(uubHistPure),-20,10]);
%             xlabel('Iteration number');
%             ylabel('Bound value');
%             title('Bound development for a case with $M = 150$ (2 clusters)');
%             
%             figure;
%             plot(vertcat(pq.score),'b');
%             hold on;
%             plot(vertcat(pq.bound),'r');
%             hold off;
%             legend('Score','Bound');
%             xlabel('Priority queue entry number');
%             ylabel('Value');
%             
%             
%             error('stop here');
