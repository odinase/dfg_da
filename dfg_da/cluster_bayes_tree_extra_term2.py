...
def conditioned_reward_matrix(R_cluster, meas_existence_mapping) -> np.ndarray:
    R = R_cluster.copy()
    n, mp1 = R.shape
    m = mp1 - 1
...
def multihypothesis_ehm2_meas_conditioned2(R_cluster, prior_hypotheses, meas_existence_mapping, reindex_tracks: bool = True):
    if reindex_tracks:
        prior_hypotheses.reindex_tracks()  # reindex tracks to {1, 2, ..., n} for later convenience
    all_tracks_idx = np.arange(R_cluster.shape[0])
    n, mp1 = R_cluster.shape
    ...
    for k, hypothesis in enumerate(prior_hypotheses):
        ...
        if existing_tracks_idx.shape[0] > 0:
            R_sub = R_cluster[existing_tracks_idx, :]
            R_conditioned = conditioned_reward_matrix(R_sub, meas_existence_mapping)
            validation_matrix, likelihood_matrix = R_LC_to_validation_likelihood_matrix(R_conditioned)
            # meas_is_gated = validation_matrix.any(axis=1)
            # if not meas_is_gated.all():
            #     JPDAprobs = np.zeros((len(existing_tracks_idx), mp1))
            #     likelihood = 0.0
            # else:
                # probs, b_likelihood = EHM2.run_and_likelihood(validation_matrix, likelihood_matrix)
            JPDAprobs, likelihood = EHM2.run_and_likelihood(validation_matrix, likelihood_matrix)
        else:
            JPDAprobs = np.zeros((0, mp1))
            likelihood = 1.0
...
class MulticlusterEfficientMarginals2:
...
class ConditionedCluster:
    ...
    def meas_conditioned_marginals(self, measurement_assignments: np.ndarray) -> np.ndarray:
        ...
        marginals_conditioned, likelihood_conditioned = multihypothesis_ehm2_meas_conditioned2(self.R_cluster, self.prior_hypotheses, meas_existence_mapping=meas_existence_mapping, reindex_tracks=False)
