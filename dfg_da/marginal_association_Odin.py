...
#     # w_nmd = (w_nmd - w_nmd[np.isfinite(w_nmd)].min()) / (w_nmd[np.isfinite(w_nmd)].max() - w_nmd[np.isfinite(w_nmd)].min())
...
#     # Init meas->track = 1, so rho is immediately the below
#     rho = 1.0 / (w_0.ravel() + (w_nmd).sum(axis=1))
...
#         rho = 1.0 / (w_0.ravel() + w_times_msg_sum.ravel())
#         # if it > 100:
#         #     sigma_new = compute_sigma(rho)
#         #     sigma = sigma_moment*sigma_new + (1 - sigma_moment)*sigma
#         #     # a2b_msg_new = w_nmd / (w_0 + (w_times_msg.sum(axis=1, keepdims=True) - w_times_msg) + sigma[:,None])
