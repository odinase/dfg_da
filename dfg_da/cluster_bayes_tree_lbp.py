...
from dfg_da.marginal_association_Odin import exact_marginal, lbp_marginal, lbp_marginal_clean, lbp_marginal_nonexistence
...
def bethe_loglikelihood(w_nmd, w_0, mu, nu, rho, sigma, phi, t2h, t2h_not) -> float:
    n, m = w_nmd.shape
...
def theta_posteriors_lbp(phi, rho, t2h_idx, t2noth_idx) -> np.ndarray:
...
def multihypothesis_lbp_bethe_meas_conditioned(R_cluster, prior_hypotheses, meas_existence_mapping, reindex_tracks: bool = True):
    if reindex_tracks:
    ...
    for k, hypothesis in enumerate(prior_hypotheses):
        ...
        if existing_tracks_idx.shape[0] > 0:
            ...
            if not meas_is_gated.all():
                ...
                lbp_probs, loglikelihood_b = lbp_marginal_clean(R_conditioned_bversion, return_mu_nu_w_nmd=True)
                JPDAprobs, likelihood = b_probs_likelihood_to_a_probs_likelihood(lbp_probs, np.exp(loglikelihood_b), R_sub)
        else:
...
class MulticlusterEfficientMarginalsLBPBethe:
    def __init__(self, R_LC, prior_hypotheses_per_cluster, assocLocal):
        ...
        # Make superclusters
        self.superclusters = [
            ConditionalSuperclusterMarginalsLBPBethe(
                R_LC=R_LC,
    ...
    def compute_marginals_likelihood(self) -> Tuple[np.ndarray, float]:
        ...
        lbp = mc.LBPMarginalsByTotalProbBethe()
        ...
        for cluster in self.cluster_links.unmerging_clusters():
            ...
            lbp_marginal_total, conditioned_theta_posterior_lbp, likelihood_lbp = lbp(R_cluster, prior_hypotheses)
            marginals[t_idxs] = lbp_marginal_total
            likelihood *= likelihood_lbp
            theta_posteriors[cluster] = conditioned_theta_posterior_lbp
...
class ConditionalSuperclusterMarginalsLBPBethe:
    def __init__(self, R_LC: np.ndarray, prior_hypotheses_per_cluster: pdd.hypothesis.HypothesesList, linking_mappings: LinkingMappings):
        ...
        self.conditioned_clusters: List[ConditionedClusterLBPBethe] = []
        for cluster, linking_measurements in linking_mappings.cluster_to_linking_measurements.items():
            ...
            self.conditioned_clusters.append(ConditionedClusterLBPBethe(
                cluster_idx=cluster,
                R_cluster=R_cluster,
...
class ConditionedClusterLBPBethe:
    ...
    def meas_conditioned_marginals(self, measurement_assignments: np.ndarray) -> np.ndarray:
        ...
        marginals_conditioned, conditioned_theta_posterior, likelihood_conditioned = multihypothesis_lbp_bethe_meas_conditioned(self.R_cluster, self.prior_hypotheses, meas_existence_mapping=meas_existence_mapping, reindex_tracks=False)
